        ...
        if self.sep_style == SeparatorStyle.ADD_COLON_SINGLE:
            ...
            for i, (role, message) in enumerate(self.messages):
                if message:
                    ...
                    ret += role + ":"
            return ret
        elif self.sep_style == SeparatorStyle.PORO:
            ret = system_prompt
            for role, message in self.messages:
                if message:
                    ret += role + message + self.sep
...
# Llama-7b-finnish-instruct-v0.2 template
# reference: https://huggingface.co/Finnish-NLP/llama-7b-finnish-instruct-v0.2
register_conv_template(
    Conversation(
        name="llama-7b-finnish-instruct",
        system_template="<|alku|>{system_message}",
        system_message="Olet tekoälyavustaja. Seuraavaksi saat kysymyksen tai tehtävän. Kirjoita vastaus parhaasi mukaan siten että se täyttää kysymyksen tai tehtävän vaatimukset.",
        roles=("<|ihminen|> Kysymys/Tehtävä:", "<|avustaja|> Vastauksesi:"),
        sep_style=SeparatorStyle.ADD_NEW_LINE_SINGLE,
        sep="\n",
        stop_token_ids=[64260],
        stop_str="<|loppu|>",
    )
)
